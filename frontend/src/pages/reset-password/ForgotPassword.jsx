...
const ForgotPasswordPage = () => {
  ...
  const { error, message, loading, success } = useSelector((state) => state.auth);
    ...
    <section className="reset-password-container">
      <form onSubmit={handleSubmit}>
        <div className="form-item" id="email">
          ...
          <input placeholder="Enter email" value={formFields.email} name="email" type="text" onChange={hanldeInputValueChange} />
        </div>
        <span className="error-text">{formErrors}</span>

        {loading ? <p className="message">Please wait ...</p> : <p className="message">{message.message}</p>}
